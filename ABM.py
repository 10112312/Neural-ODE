...
class SIR_ABM:
    def __init__(
        ...
        **__,
        ...
        :param beta: the intensity of encounters
        :param gamma: I->R: exponentially distribution time with rate gamma
        :param eta: transition rate from R to S
        """
    ...
    def run_single(self, *, parameters: torch.tensor = None):
        ...
        # beta = self.beta if parameters is None else parameters[2]
        # gamma = self.gamma if parameters is None else parameters[3]
        # eta = self.eta if parameters is None else parameters[4]
